...
do
        ...
        echo "ssh dmr${nodenumber} /opt/hpc/build/bin/start-munge.sh"
        ssh dmr${nodenumber} /opt/hpc/build/bin/start-munge.sh || true
done
...
do
        nodenumber=$(printf "%02d" ${h})
        echo "ssh dmr${nodenumber} /opt/hpc/build/bin/start-slurmd.sh"
        ssh dmr${nodenumber} /opt/hpc/build/bin/start-slurmd.sh || true
done
...
/opt/hpc/install/slurm-dmr/sbin/slurmctld
sleep 5

echo "Checking with \"sinfo\""
echo ""
/opt/hpc/install/slurm-dmr/bin/sinfo
